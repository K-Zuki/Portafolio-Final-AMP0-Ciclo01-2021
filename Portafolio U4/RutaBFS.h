...
class RutaBFS{
	private:
		const int INFINITO = std::numeric_limits<int>::max();	// El valor maximo que puede tener un Int
		int vertices;						// numero de vertices del grafo de origen
		...
		void bfs(const Digrafo &G, int s);	// algoritmo Breadth First Search busqueda en anchura recursiva
	public:
		RutaBFS(const Digrafo &G, int s);	// constructor a partir de digrafo
		RutaBFS(const RutaBFS &otra);		// constructor a partir de otra busqueda de rutas
		~RutaBFS();							// destructor
		bool existeCaminoHacia(int v);		// saber si existe un camino entre un punto y otro
		int getDistanciaHacia(int v);		// obtener la distancia al camino si es que existe
		list<int> getRutaHacia(int v);		// obtener la ruta si existe
};
